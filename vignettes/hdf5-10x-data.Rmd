...
```{r}
tdb_array <- tiledb_array(tdb_uri, return_as = "matrix")
```
...
In order to use *Seurat* to analyze 10x count data stored with TileDB you'll need to read the dataset into memory

```{r}
count_mat <- Read10x_tiledb(tdb_uri, verbose = TRUE)
```

and create a Seurat object:

```{r}
counts <- Seurat::CreateSeuratObject(counts = count_mat, project = "pbmc3p")
counts
```
