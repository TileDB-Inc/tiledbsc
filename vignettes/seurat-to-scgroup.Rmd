...
scgroup_x$from_matrix(
  x = GetAssayData(object = pbmc_small, slot = "counts")
)
```
...
scgroup_var$from_dataframe(
  x = GetAssay(pbmc_small, assay = "RNA")[[]]
)
```
...
Then we can then pass the entire `pbmc_small` object directly to `from_seurat()` and one `sc_group` will be created for each `Assay` object:
