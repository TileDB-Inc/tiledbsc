...
implementation of this API allows
...
```{r}
scdataset <- SCDataset$new(uri = file.path(tempdir(), "pbmc_small"))
```

This indicates that no dataset currently exists at the URI specified and that a new one should be created. At this point, `pbmc_small` contains only a TileDB group marker file and an empty metadata array.
