...
As an example, let's access the cell-level metadata. Recall from the [`matrix-api spec`][matrix-api-spec] that cell-level metadata is stored in the `obs` array of an `SOMA`. Therefore, we must first access a specific `SOMA` within the `SOMACollection`'s `somas` slot. Let's generate a list of the available `somas`:
