...
```{r}
tdb_10x_uri <- tools::file_path_sans_ext(h5_file)

tdb_assay <- TileDBAssay$new(uri = tdb_10x_uri, file_path = h5_file)
```
...
```{r}
tdb_array <- tdb_assay$tiledb_array(return_as = "tibble")
```
...
selected_ranges(tdb_array) <- list(var_id = cbind(genes, genes))

# run the query
tbl_counts <- tdb_array[]

boxplot(data ~ var_id, data = tbl_counts, col = "skyblue", pch = 16)
```
...
```{r}
pbmc3p <- CreateSeuratObject(counts = count_mat, project = "pbmc3p")
```
